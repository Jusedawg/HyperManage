...
In single-player, use **Review refunds** in the right panel to estimate the selected buildings' dismantle returns. The target is excluded. The review combines ordinary buildings and lightweight pieces, reports additional native child buildings, and lists item totals (up to 12 types). No buildings are removed, no items are granted, and undo history is unchanged.
