...
The review also checks ordinary buildings and their added children for current dismantle eligibility. It shows how many currently refuse dismantling and all distinct warning reasons reported by the game. Warnings do not automatically mean removal is blocked. Refund estimates remain visible when a building refuses dismantling, since the review changes nothing. Lightweight removal eligibility is explicitly marked as unchecked. Conditions may change, including after dependencies are removed; these diagnostics do not authorize deletion.
...
The refund report slides left from behind the main tool tray in a matching panel, inset at the top and bottom with its right edge tucked under the main rim. It starts closed and has its own scrollbar, **Refresh** button and **X** close button. Closing the report leaves the main tray open. Refresh replaces the report using your current selection and inventory; the time shown is the last refresh, not a live feed. A failed refresh replaces old totals with the error. Changing the selection or protected target, losing a reviewed object, changing a reviewed transform, updating edit history (including undo/redo), or starting a pending building edit replaces old totals with an out-of-date notice. Refresh to calculate again. Inventory and machine contents still require manual refresh. The report is not saved with the game.
