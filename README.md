...
The refund report slides left from behind the main tool tray in a matching panel, inset at the top and bottom with its right edge tucked under the main rim. It starts closed and has its own scrollbar, **Refresh** button and **X** close button. Closing the report leaves the main tray open. Refresh replaces the report using your current selection and inventory; the time shown is the last refresh, not a live feed. A failed refresh replaces old totals with the error. The report is not saved with the game.
