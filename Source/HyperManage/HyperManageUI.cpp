...
void UHyperManageUI::NextMMWidget()
{
	CurrentWidgetType = static_cast<EWidgetPanel>((CurrentWidgetType + 1) % 2);
	ShowMMWidget();
}

void UHyperManageUI::HideMMWidget()
{
...
void UHyperManageUI::ShowMMWidget()
{
	HideMMWidget();

	if (CurrentWidgetType == EWidgetPanel::InfoBoard) {
		CurrentWidget = CreateWidget<UHyperManageClipboardWidget>(System->GetLocalController(), UHyperManageClipboardWidget::StaticClass());
		if (CurrentWidget) CurrentWidget->AddToViewport(-1);
		return;
	}
	FSoftClassPath WidgetClassReference;

	switch (CurrentWidgetType) {
	case EWidgetPanel::Placeholder:
		WidgetClassReference = FSoftClassPath(TEXT(PLACEHOLDER_WIDGET_CLASS));
		break;
	case EWidgetPanel::InfoBoard:
		WidgetClassReference = FSoftClassPath(TEXT(CLIPBOARD_WIDGET_CLASS));
		break;
	}
...
void UHyperManageUI::ShowMessage(const FString& Message)
{
...
void UHyperManageUI::PopupClosed(bool ConfirmClicked)
{

}

void UHyperManageUI::ShowPopup(const FString& Title, const FString& Message)
{
...
void UHyperManageUI::ShowConfirm(const FString& Title, const FString& Message, UObject* Object, const FName& FuncName)
{
	FPopupClosed ClosedDelagate;
...
void UHyperManageUI::ComingSoon()
{
	ShowPopup(TITLE_COMING_SOON, BODY_COMING_SOON);
}

void UHyperManageUI::ShowToolsUI()
{
	FSoftClassPath WidgetClassReference = FSoftClassPath(TEXT(TOOLBAR_WIDGET_CLASS));
