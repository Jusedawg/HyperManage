...
foreach ($test in @('StaleIdentity', 'TransformBounds', 'PivotTransforms', 'AttachmentRoot', 'PreservesMaterials', 'WorldAlignment', 'NestedToolbar', 'OriginalClipboard', 'PrecisionValues', 'UndoRedo', 'WorldOffset', 'WorldRotationOffset', 'ExactScale', 'SelectionChanges', 'OriginAlignment', 'HeightGrid', 'WorldPosition', 'WorldOrientation', 'DependencyPlan', 'RefundPreview', 'LightweightRefund')) {
    if ($report -notmatch "Test Completed\. Result=\{Success\} Name=\{$test\}") { throw "Test $test did not pass. See $log" }
}
Write-Output 'All twenty-one HyperManage regression tests passed.'
