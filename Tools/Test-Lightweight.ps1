...
foreach ($test in @('StaleIdentity', 'TransformBounds', 'PivotTransforms', 'AttachmentRoot', 'PreservesMaterials', 'WorldAlignment', 'NestedToolbar', 'OriginalClipboard', 'PrecisionValues', 'UndoRedo')) {
    if ($report -notmatch "Test Completed\. Result=\{Success\} Name=\{$test\}") { throw "Test $test did not pass. See $log" }
}
Write-Output 'All ten HyperManage regression tests passed.'
